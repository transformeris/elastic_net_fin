...
if __name__ == '__main__':
    ...
    for i in range(2, len(date)):
        ...
        # if jcm_delta[date[i]]>0 and jcl_delta[date[i]]>0 and mms[date[i]] > mmm[date[i]]:
            # mm_duanxian_buy.append(mms.index[i])
            mm_duanxian_buy.append(date[i])

    jinzi_buy = jinzi[mm_duanxian_buy]
    jinzi_sell = jinzi_shift[mm_duanxian_buy]
