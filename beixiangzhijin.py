...
def get_data_proto(page):
    ...
    for i in page:
        ...
        url = 'http://dcfm.eastmoney.com/em_mutisvcexpandinterface/api/js/get?callback=jQuery112309149170311082313_1625319963468&st=DetailDate&sr=-1&ps=10&p=' + str(
            i) + '&type=HSGTHIS&token=894050c76af8597a853f5b408b759f5d&js=%7Bpages%3A(tp)%2Cdata%3A(x)%7D&filter=(MarketType%3D1)'
        try:
            req = requests.get(url, headers=header)
            html = req.content.decode("utf-8")
