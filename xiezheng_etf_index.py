...
if __name__=='__main__':
    etf=load_obj('etf_all')
    etf_50=etf['sh510050']
    index_50 = ak.stock_zh_index_daily(symbol="sh000016")
    index_50.set_index('date',inplace=True)
    zz=etf_50[datetime.date(2019,1,1):datetime.date(2019,12,31)]
    zzz=index_50[datetime.date(2019,1,1):datetime.date(2019,12,31)]
    zz['close_2']=zz['close']*(zzz.loc[datetime.date(2019,1,2),'close']/zz.loc[datetime.date(2019,1,2),'close'])
    zzzz=zz['close_2']-zzz['close']
    x=zzz['close']
    y=zz['close_2']

    etf_test=etf_50[datetime.date(2017,1,1):datetime.date(2021,12,31)]
    index_test = index_50[datetime.date(2017, 1, 1):datetime.date(2021, 12, 31)]

    etf_test_check=etf_test['close']
    index_test_check=index_test['close']*(etf_50.loc[datetime.date(2017, 1, 3),'close']/index_50.loc[datetime.date(2017, 1, 3),'close'])

    etf_sign=copy.deepcopy(etf_test_check)
    etf_sign[etf_sign>0]=0
    etf_sign[etf_test_check<index_test_check]=1

    etf_50.loc[:,'sign']=etf_sign