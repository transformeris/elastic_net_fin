...
if __name__ == '__main__':
    ...
    for i in range(2, len(date)):
        ...
        if (mmm[date[i]] > mml[date[i]] and mmm[date[i - 1]] < mml[date[i - 1]]) and jcl_delta[date[i]]>0 and jcl_delta[date[i-1]]>0 and jcm_delta[date[i]]>0 and jcm_delta[date[i-1]]>0:
    ...
    for i in range(2, len(date)):
        # if (jcs[date[i]]>jcm[date[i]]>jcl[date[i]] and jcl_delta[date[i]]>0 and jcm_delta[date[i]]>0) and (mms[date[i]] > mmm[date[i]] and jcl_delta[date[i]]>0 and jcm_delta[date[i]]>0):
        # if bias_mm_s_l[date[i]] > 1.5*flag[date[i]] and (mms[date[i]] > mmm[date[i]] and mms[date[i - 1]] < mmm[date[i - 1]]):
        if (mmm[date[i]] < mml[date[i]] and mmm[date[i - 1]] > mml[date[i - 1]]):
            # mm_duanxian_buy.append(mms.index[i])
            mm_duanxian_sell.append(date[i])
            data.loc[date[i], '收盘信号'] = 0
    data = data[0:len(data)]
    import matplotlib.pyplot as plt
    ...
    for i in range(len(data)):
        if data['收盘信号'][i] == 0:
            plt.annotate('卖', xy=(i, data['收盘'][i]), arrowprops=dict(facecolor='r', shrink=0.01))
    plt.show()
