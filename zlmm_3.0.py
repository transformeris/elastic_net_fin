...
if __name__=='__main__':
    ...
    for i in range(1,len(mms)):
        if (mms.iloc[i]>mmm.iloc[i] and mms.iloc[i-1]<mmm.iloc[i-1]) and (mms.iloc[i]>mml.iloc[i] and mms.iloc[i-1]<mml.iloc[i-1]) and jcm_delta[i]>0 and jcl_delta[i]>0:
    ...
    for i in range(1, len(mms)):
        if
    # for i in range(1,len(mms)):
    #     if (mms.iloc[i]<mmm.iloc[i] and mms.iloc[i-1]>mmm.iloc[i-1]) and (mmm.iloc[i]<mml.iloc[i] and mmm.iloc[i-1]>mml.iloc[i-1]):
