...
if __name__=='__main__':
    ...
    for i in range(2,len(date)):
        ...
        if bias_mm_s_l[date[i]] < flag[date[i]] and (mms[date[i]] > mmm[date[i]] and mms[date[i - 1]] < mmm[date[i - 1]]):
            #mm_duanxian_buy.append(mms.index[i])
            mm_duanxian_buy.append(date[i])
            print(mms.index[i])
            print('fuck',date[i])
    ...
    for i in range(2, len(date)):
        if bias_mm_s_l[date[i]] < flag[date[i]] or (mms[date[i]] < mmm[date[i]] and mms[date[i - 1]] > mmm[date[i - 1]]):
    ...
    # plt.plot(df['指数净值'])
    # plt.show()
    df = huiche(data, mm_duanxian_buy,['2005-01-06'])
    data = df[2000:4294]
    import matplotlib.pyplot as plt
