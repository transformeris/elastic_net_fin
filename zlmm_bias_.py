...
if __name__ == '__main__':
    ...
    for i in range(2, len(date)):
        ...
        if bias_mm_s_l[date[i]] > 1.5 * flag[date[i]] and (
                mms[date[i]] > mmm[date[i]] and mms[date[i - 1]] < mmm[date[i - 1]]) and bias_jc_s_l[date[i]] > 10 * flag2[date[i]]:
            # mm_duanxian_buy.append(mms.index[i])
            mm_duanxian_buy.append(date[i])
            print(mms.index[i])
            print('fuck', date[i])
    ...
    sell=qushi_delte[qushi_delte<-0.01*qushi]
    mm_duanxian_sell=list(sell.index)
