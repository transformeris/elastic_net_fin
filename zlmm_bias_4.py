...
if __name__ == '__main__':
    ...
    for i in range(2, len(date)):
        ...
        if bias_mm_s_l[date[i]] > 1 * flag[date[i]] and (
                mms[date[i]] > mmm[date[i]] and mms[date[i - 1]] < mmm[date[i - 1]]) and bias_jc_s_l[date[i]] > 0:
            # mm_duanxian_buy.append(mms.index[i])
            mm_duanxian_buy.append(date[i])
            print(mms.index[i])
            print('fuck', date[i])
    ...
    mm_duanxian_sell = []
    qushi=data['收盘'].rolling(window=20).mean()

    qushi_delte=qushi.diff()
    mm_duanxian_sell = []
    # mm_duanxian_sell=list(set(list(date))-set(mm_duanxian_buy))
    for i in range(2, len(date)):
        # if bias_mm_s_l[date[i]] < min(-10,1.5*flag[date[i]]) and (mms[date[i]] < mmm[date[i]] and mms[date[i - 1]] > mmm[date[i - 1]]) and bias_jc_s_l[date[i]] < 1.5*flag2[date[i]]:
        if bias_mm_s_l[date[i]] > 1.5 * flag[date[i]] and (
                mms[date[i]] < mmm[date[i]] and mms[date[i - 1]] > mmm[date[i - 1]]) and bias_jc_s_l[date[i]] > 3 * \
                flag2[date[i]]:
