...
def zhibiao():
    ...
    jcs_rolling_3=var3.rolling(window=3).mean()
    ...
    return data,jcs,jcm,jcl,mms,mmm,mml,jcs_rolling_3
...
if __name__=='__main__':
    data,jcs, jcm, jcl, mms, mmm, mml,jcs_rolling_3=zhibiao()
    bias_jc_s_m = ((jcs - jcm) / jcm) * 100
    bias_jc_s_l = ((jcs - jcl) / jcl) * 100
    jcm_delta = jcm.diff()
    jcl_delta = jcl.diff()
    jcs_delta=jcs_rolling_3.diff()
    mms_delta=mms.diff()
    mmm_delta=mmm.diff()
    mml_delta=mml.diff()
    data_buy=[]
    ...
    for i in range(2,len(date)):
        if (mms[date[i]]>mml[date[i]] and mms[date[i-1]]<mml[date[i-1]] and mmm[date[i]]>mml[date[i]] and jcm_delta[date[i]]>0 and jcl_delta[date[i]]>0 and jcs_delta[date[i]]>0) or (mmm[date[i]]>mml[date[i]] and mmm[date[i-1]]<mml[date[i-1]] and mms[date[i]]>mml[date[i]] and jcm_delta[date[i]]>0 and jcl_delta[date[i]]>0 and jcs_delta[date[i]]>0):
    ...
    for i in range(1, len(date)):
        if (mms[date[i]] < mmm[date[i]] and mms[date[i - 1]] > mmm[date[i - 1]]) and jcm_delta[date[i]]<=0 and jcl_delta[date[i]]<=0 and jcs_delta[date[i]]<=0:
