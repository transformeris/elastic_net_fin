...
if __name__=='__main__':
    ...
    for i in range(1,len(date)):
        if ((mms[date[i]]>mmm[date[i]] and mms[date[i-1]]<mmm[date[i-1]]) and jcm_delta[date[i]]>0 and jcl_delta[date[i]]>0) or (mms[date[i-1]]<mms[date[i]] and mms[date[i-2]]>mms[date[i-1]] and mms[date[i]]<mmm[date[i]] and mml_delta[date[i]]<0 and jcm_delta[date[i]]>0 and jcl_delta[date[i]]>0 and bias_mm_m_s[date[i]]<2 and bias_mm_m_s[date[i]]<2):
