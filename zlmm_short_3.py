...
if __name__=='__main__':
    ...
    for i in range(2,len(date)):
        #if (bias_mm_s_m[date[i - 1]]<bias_mm_s_m[date[i]] and abs(bias_mm_s_m[date[i]])<2 and jcm_delta[date[i]]>0 and jcl_delta[date[i]]>0):
    ...
    for i in range(2, len(date)):
        if (bias_mm_s_m[date[i - 1]]>bias_mm_s_m[date[i]] and bias_mm_s_l[date[i]]<4 and jcm_delta[date[i]]<0 and jcl_delta[date[i]]<0) or bias_mm_s_l[date[i]]>80:
